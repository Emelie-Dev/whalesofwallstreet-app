//! Per-IP request throttling.
//!
//! The `/api/v1/quote` handler alone runs a bounded but non-trivial
//! priority-queue search (`RoutePlanner::find_best_route`) across every
//! chain/asset combination per request. With no throttling anywhere in
//! `create_router`, it — and every other route — is open to trivial
//! volumetric abuse from any origin. [`RateLimiter`] enforces a simple
//! fixed-window per-key budget; [`rate_limit_middleware`] wires it up as
//! Axum middleware.

use crate::error::AppError;
use axum::{
    extract::{ConnectInfo, Request, State},
    http::header,
    middleware::Next,
    response::Response,
};
use moka::future::Cache;
use std::net::SocketAddr;
use std::sync::atomic::{AtomicU32, Ordering};
use std::sync::Arc;
use std::time::Duration;

/// Fixed-window per-key request counter.
///
/// Each key's counter lives for exactly `window` from the moment it's first
/// created (moka's per-entry TTL); once the entry is evicted the window
/// resets. This mirrors [`crate::bridge::gas_oracle::GasOracle`]'s
/// TTL-cache pattern rather than pulling in a dedicated rate-limiting crate.
#[derive(Clone)]
pub struct RateLimiter {
    counters: Cache<String, Arc<AtomicU32>>,
    limit_per_window: u32,
    window: Duration,
    /// Whether `X-Forwarded-For` may be trusted to recover the real client
    /// IP. Must only be `true` when this engine sits behind a proxy that is
    /// actually configured to overwrite/strip that header from inbound
    /// client traffic — otherwise any client can set it themselves and
    /// spoof its way to a fresh rate-limit bucket. See
    /// [`crate::config::AppConfig::trust_proxy_headers`].
    trust_proxy_headers: bool,
}

impl RateLimiter {
    pub fn new(limit_per_window: u32, window: Duration, trust_proxy_headers: bool) -> Self {
        Self {
            counters: Cache::builder().time_to_live(window).build(),
            limit_per_window,
            window,
            trust_proxy_headers,
        }
    }

    /// Returns `Ok(())` if `key` is still within budget for its current
    /// window, or `Err(retry_after)` once the budget is exhausted.
    async fn check(&self, key: &str) -> Result<(), Duration> {
        let counter = self
            .counters
            .get_with(key.to_string(), async { Arc::new(AtomicU32::new(0)) })
            .await;

        let count = counter.fetch_add(1, Ordering::SeqCst) + 1;
        if count > self.limit_per_window {
            Err(self.window)
        } else {
            Ok(())
        }
    }

    /// Client identity to throttle on: the real TCP peer IP (wired up via
    /// `into_make_service_with_connect_info` in `main`) whenever it's
    /// available, since that's never spoofable by the request itself.
    ///
    /// `X-Forwarded-For` is consulted *only* when `trust_proxy_headers` is
    /// set — an end client can set that header to anything it likes, so
    /// honoring it unconditionally would let anyone bypass their own
    /// rate-limit bucket by sending a fresh value on every request.
    fn client_key(&self, req: &Request) -> String {
        if let Some(ConnectInfo(addr)) = req.extensions().get::<ConnectInfo<SocketAddr>>() {
            return addr.ip().to_string();
        }

        if self.trust_proxy_headers {
            if let Some(key) = req
                .headers()
                .get(header::HeaderName::from_static("x-forwarded-for"))
                .and_then(|v| v.to_str().ok())
                .and_then(|v| v.split(',').next())
                .map(|v| v.trim().to_string())
                .filter(|v| !v.is_empty())
            {
                return key;
            }
        }

        // No real peer address available (e.g. driven via `oneshot` in
        // tests, or without a trusted proxy header) and nothing safe to key
        // on: everyone shares one bucket rather than going unlimited.
        "unknown".to_string()
    }
}

pub async fn rate_limit_middleware(
    State(limiter): State<RateLimiter>,
    req: Request,
    next: Next,
) -> Result<Response, AppError> {
    let key = limiter.client_key(&req);
    match limiter.check(&key).await {
        Ok(()) => Ok(next.run(req).await),
        Err(retry_after) => Err(AppError::TooManyRequests(retry_after.as_secs().max(1))),
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[tokio::test]
    async fn allows_up_to_the_limit_then_rejects() {
        let limiter = RateLimiter::new(3, Duration::from_secs(60), false);
        assert!(limiter.check("1.2.3.4").await.is_ok());
        assert!(limiter.check("1.2.3.4").await.is_ok());
        assert!(limiter.check("1.2.3.4").await.is_ok());
        assert!(limiter.check("1.2.3.4").await.is_err());
    }

    #[tokio::test]
    async fn tracks_separate_budgets_per_key() {
        let limiter = RateLimiter::new(1, Duration::from_secs(60), false);
        assert!(limiter.check("1.2.3.4").await.is_ok());
        assert!(limiter.check("5.6.7.8").await.is_ok());
        assert!(limiter.check("1.2.3.4").await.is_err());
    }

    fn request_with_xff(value: &str) -> Request {
        axum::http::Request::builder()
            .uri("/")
            .header("x-forwarded-for", value)
            .body(axum::body::Body::empty())
            .unwrap()
    }

    #[test]
    fn ignores_x_forwarded_for_by_default() {
        let limiter = RateLimiter::new(10, Duration::from_secs(60), false);
        let req = request_with_xff("1.2.3.4");
        // No ConnectInfo extension present (as in a spoofed/unproxied
        // request reaching this middleware directly) and proxy headers are
        // not trusted, so every such request collapses onto the shared
        // fallback bucket rather than letting the header pick a fresh one.
        assert_eq!(limiter.client_key(&req), "unknown");
    }

    #[test]
    fn honors_x_forwarded_for_only_when_trust_proxy_headers_is_set() {
        let limiter = RateLimiter::new(10, Duration::from_secs(60), true);
        let req = request_with_xff("203.0.113.7, 10.0.0.1");
        assert_eq!(limiter.client_key(&req), "203.0.113.7");
    }

    #[test]
    fn connect_info_always_wins_over_x_forwarded_for() {
        let limiter = RateLimiter::new(10, Duration::from_secs(60), true);
        let mut req = request_with_xff("203.0.113.7");
        req.extensions_mut()
            .insert(ConnectInfo(SocketAddr::from(([127, 0, 0, 1], 12345))));
        assert_eq!(limiter.client_key(&req), "127.0.0.1");
    }
}
